...
- .css 网页样式文件。
- .js 网页JavaScript脚本文件。
- .brs 存储浏览序列文件 - 可以使用记事本打开。
- .hhc 存储的是由Microsoft帮助内容/网站导航表 - 可以使用记事本打开。
- .hhk 它存储的是由Microsoft帮助的索引 - 可以使用记事本打开。
- .glo 存储词汇表 - 可以使用记事本打开
- .ing 非重要文件。 
...
9. 翻译需要保证''["， "]"和"#"字符不得丢失，可以通过如下命令实现第5至9步的内容，
...
```
(?<=name="Name" value=")[A-Za-z0-9_,;&\s\S\.\-\/]+(?=">)
```
